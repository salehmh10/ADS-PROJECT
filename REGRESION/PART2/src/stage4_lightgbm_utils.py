"""Shared helpers for Stages 4F-H of the LightGBM track.

The module reads only saved Train row IDs. It never loads locked Test values or
creates Test predictions. Every learned transform is fitted inside one Pipeline.
"""
...
STAGE_NAMES = {
    "stage4f": "Stage 4F — Initial LightGBM Model and Importance Analysis",
    "stage4g": "Stage 4G — LightGBM Feature Confirmation and Final Tuning",
    "stage4h": "Stage 4H — Final LightGBM Sensitive Comparison and Full-Train Models",
}
VERSION = "stage4f_stage4h_lightgbm_v1_20260714"
...
def paths(root: str | Path) -> dict[str, Path]:
    ...
    return {
        "root": project,
        "initial": project / "artifacts/results/stage4/lightgbm/initial",
        "confirmation": project / "artifacts/results/stage4/lightgbm/feature_confirmation",
        "final": project / "artifacts/results/stage4/lightgbm/final",
        "predictions": project / "artifacts/predictions/lightgbm",
        "preliminary_models": project / "artifacts/models/lightgbm/preliminary",
        "final_models": project / "artifacts/models/lightgbm/final",
        "candidate_models": project / "artifacts/models/lightgbm/candidates",
        "features": project / "artifacts/features/stage4/lightgbm",
        "figures": project / "artifacts/figures/stage4/lightgbm",
        "checkpoints": project / "artifacts/checkpoints/stage4/lightgbm",
        "manifests": project / "artifacts/manifests/stage4/lightgbm",
...
class LightGBMFeatureEngineerV2(BaseEstimator, TransformerMixin):
    """Preserve Stage 4B fixed Features and add approved Stage 4G Features."""
    ...
    def fit(self, X: pd.DataFrame, y: Any = None) -> "LightGBMFeatureEngineerV2":
        if not isinstance(X, pd.DataFrame):
            raise TypeError("X must be a pandas DataFrame.")
        selected = tuple(self.selected_proposals)
        unknown = sorted(set(selected).difference(SAFE_PROPOSALS))
        if unknown:
            raise ValueError(f"Unknown Stage 4G proposals: {unknown}")
        required = {
        ...
        if PROPOSAL_INCOME_LIEN in selected:
            ...
            if not len(finite):
                raise ValueError("Income-band Feature needs finite training income values.")
    ...
    def transform(self, X: pd.DataFrame) -> pd.DataFrame:
        ...
        if not hasattr(self, "selected_proposals_"):
            raise RuntimeError("LightGBMFeatureEngineerV2 is not fitted.")
...
def feature_pack_spec(root: str | Path, pack_id: str, selected_proposals: Sequence[str] = ()) -> dict[str, Any]:
    base_id = "lightgbm_encoded_v1" if pack_id.startswith("lightgbm_encoded_v2") else pack_id
    ...
    if proposals:
        ...
        spec["categorical"] = list(dict.fromkeys(categorical))
...
class LightGBMModelBundle:
    """Serializable complete bundle that accepts raw named DataFrames."""
    ...
    def predict(self, X: pd.DataFrame) -> np.ndarray:
        ...
        output = self.pipeline.predict(X.loc[:, raw].copy())
...
def prepare_pipeline(
    ...
    selected = model_numeric + categorical
    proposals = tuple(spec.get("selected_proposals", ()))
    engineer: Any = LightGBMFeatureEngineerV2(proposals) if proposals else s4.Stage4FixedFeatureEngineer()
    steps: list[tuple[str, Any]] = [("features", engineer)]
    sanitize_columns = list(dict.fromkeys(categorical + frequency_sources))
    if sanitize_columns:
    ...
    steps.append(("model", LGBMRegressor(**model_parameters)))
    return Pipeline(steps), raw, selected, categorical
...
def fit_validation(
    ...
    fit_kwargs: dict[str, Any] = {"eval_set": [(validation_ready, y_eval)], "eval_metric": "mae"}
    if early_stopping_rounds is not None:
        from lightgbm import early_stopping, log_evaluation

        fit_kwargs["callbacks"] = [early_stopping(int(early_stopping_rounds), verbose=False), log_evaluation(0)]
    ...
    best_iteration = int(model.best_iteration_) if getattr(model, "best_iteration_", 0) else int(parameters["n_estimators"])
    return {
        "pipeline": fitted,
        "raw_columns": raw,
        ...
        "feature_names": fitted.named_steps["preprocess"].get_feature_names_out().tolist(),
    }
...
def fit_full_train(
    ...
    started = time.perf_counter()
    pipeline.fit(X, s4.transform_target(y, target_mode))
    fit_seconds = time.perf_counter() - started
    reference_ids = ids[: min(500, len(ids))]
    reference_prediction = s4.inverse_target(pipeline.predict(frame.loc[reference_ids, raw].copy()), target_mode)
    return {
        "pipeline": pipeline,
        "raw_columns": raw,
        "selected_features": selected,
...
def atomic_native_save(model: Any, path: str | Path) -> None:
    ...
    if not temporary.is_file() or temporary.stat().st_size <= 0:
        raise IOError("Native Booster temporary file is invalid.")
...
def validate_checkpoint(root: str | Path, checkpoint_path: str | Path) -> dict[str, Any]:
    ...
    hash_fields = {
        ...
        "native_model_path": "native_model_sha256",
        "prediction_path": "prediction_sha256",
    }
    for key, hash_key in hash_fields.items():
    ...
    reason = "ok" if valid else f"missing_artifacts: {missing}; mismatches: {mismatches}"
    return {"valid": valid, "reason": reason, "path": str(path), "checkpoint": data}


def registry_row(checkpoint: dict[str, Any], evaluation_stage: str, notes: str = "") -> dict[str, Any]:
    """Convert one validated checkpoint to the fixed project Registry schema."""
...
def upsert_registry_preserve_prior(root: str | Path, rows: Sequence[dict[str, Any]], allowed_stage_ids: Sequence[str]) -> pd.DataFrame:
    """Upsert owned rows while preserving every non-owned CSV record byte-for-byte."""
    project = Path(root).resolve()
    ...
    for record in records[1:]:
        if not record:
            continue
        experiment_id = next(csv.reader([record]))[0]
        if experiment_id not in owned_ids:
            kept.append(record)
    generated: list[str] = []
    ...
    output = newline.join(kept + generated) + newline
    temporary = path.with_name(path.name + ".tmp")
    temporary.write_bytes(output.encode("utf-8"))
...
def clean_reload_check(root: str | Path, model_path: str | Path, native_path: str | Path, prediction_path: str | Path) -> dict[str, Any]:
    ...
    bundle: LightGBMModelBundle = joblib.load(model_path)
    reference = pd.read_csv(prediction_path, dtype={"row_id": "int64"}).head(500)
    raw = list(bundle.metadata["raw_columns"])
    source = Path(bundle.metadata["source_path"])
    frame = s4.read_training_rows(source, reference["row_id"].to_numpy(dtype=np.int64), raw)
    ...
    ready = Pipeline(pipeline.steps[:-1]).transform(X)
    native = Booster(model_file=str(native_path))
    native_prediction = s4.inverse_target(native.predict(ready), bundle.target_mode)
    result = {
        ...
        "reference_prediction_match": bool(np.allclose(prediction, expected, rtol=1e-10, atol=1e-10)),
        "native_prediction_match": bool(np.allclose(native_prediction, expected, rtol=1e-9, atol=1e-9)),
        "inverse_target_verified": bundle.target_mode in {"raw", "log1p"},
        "native_booster_loaded": True,
    }
    result["status"] = "PASS" if all(value for key, value in result.items() if key not in {"model_path", "native_model_path", "prediction_path", "rows", "status"}) else "FAIL"
    return result
